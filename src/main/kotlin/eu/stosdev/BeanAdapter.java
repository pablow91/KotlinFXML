...
public class BeanAdapter extends AbstractMap<String, Object> {
    ...
    private final HashMap<String, Object> propertiesToSet = new HashMap<>();
    ...
    private static MethodCache getClassMethodCache(final Class<?> type) {
        ...
        synchronized (globalMethodCache) {
            ...
            if (Modifier.isPublic(type.getModifiers())) {
                ...
                final Method[] declaredMethods = AccessController.doPrivileged((PrivilegedAction<Method[]>) type::getDeclaredMethods);
     ...
     * @return The value returned by the method, or <tt>null</tt> if no such method
     * exists.
     */
    @Override
     ...
     * @return Returns <tt>null</tt>, since returning the previous value would require
     * an unnecessary call to the getter method.
     * @throws PropertyNotFoundException If the given property does not exist or is read-only.
     */
    @Override
    public Object put(String key, Object value) {
        ...
        try {
            //System.out.println("Setting " +  + " for " + bean + "with method " + setterMethod);
            propertiesToSet.put(setterMethod.getName(), coerce(value, getType(key)));
    ...
    public HashMap<String, Object> getPropertiesToSet() {
     ...
     *
     * @param key The property name.
     * @return <tt>true</tt> if the property is read-only; <tt>false</tt>, otherwise.
     */
     ...
     *
     * @param key The property name.
     * @return The named property model, or <tt>null</tt> if no such property exists.
     */
     ...
     * @param key        The property name.
     * @param value      The new property value.
     * @throws PropertyNotFoundException If the given static property does not exist or is read-only.
     */
        ...
        if (targetType != null) {
            ...
            if (method == null) {
                ...
                for (Class<?> anInterface : interfaces) {
                    try {
                        method = MethodUtil.getMethod(sourceType, getMethodName, new Class[]{anInterface});
                    } catch (NoSuchMethodException exception) {
                        // No-op
                    }
                    ...
                    if (method == null) {
                        try {
                            method = MethodUtil.getMethod(sourceType, isMethodName, new Class[]{anInterface});
                        } catch (NoSuchMethodException exception) {
                            // No-op
                        }
                    }
    ...
    private static Method getStaticSetterMethod(Class<?> sourceType, String key, Class<?> valueType, Class<?> targetType) {
        ...
        if (targetType != null) {
            ...
            if (method == null) {
                ...
                for (Class<?> anInterface : interfaces) {
                    try {
                        method = MethodUtil.getMethod(sourceType, setMethodName, new Class[]{anInterface, valueType});
                    } catch (NoSuchMethodException exception) {
                        // No-op
                    }
