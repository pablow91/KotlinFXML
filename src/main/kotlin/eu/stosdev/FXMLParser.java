...
public class FXMLParser {
    ...
    private abstract class Element {
        ...
        public List<Object> childListItems = new ArrayList<>();
        ...
        public void set(Object value) throws LoadException {
            ...
            childProperty = defaultProperty.value();
            childObject = value;
            ...
            if (aValue.startsWith(ESCAPE_PREFIX)) {
                ...
                if (aValue.startsWith(RELATIVE_PATH_PREFIX)) {
                    ...
                    if (aValue.charAt(0) == '/') {
                        ...
                        try {
                            return new URL(FXMLParser.this.location, aValue).toString();
                        } catch (MalformedURLException e) {
                            System.err.println(FXMLParser.this.location + "/" + aValue);
                        }
                    }
                }
        ...
        public void processEventHandlerAttributes() throws LoadException {
            if (eventHandlerAttributes.size() > 0) {
                ...
                if (handlerValue.startsWith(CONTROLLER_METHOD_PREFIX)) {
                    ...
                    if (handler != null) {
                        if (handler.type == SupportedType.EVENT) {
                            ...
                            propertyModel.addListener((observable, oldValue, newValue) -> {
    ...
    private abstract class ValueElement extends Element {
        ...
        public void processEndElement() throws IOException {
            ...
            String objectId;
            if (((Node) value).getId() != null) {
                objectId = ((Node) value).getId();
                constructorBuilder.append(objectId).append(" = ").append(value.getClass().getSimpleName()).append("()").append('\n');
            } else {
                objectId = "object" + objCounter;
                ((Node) value).setId(objectId);
                constructorBuilder.append("val ").append(objectId).append(" = ").append(value.getClass().getSimpleName()).append("()").append('\n');
                objCounter++;
            }
            getValueAdapter().getPropertiesToSet().forEach((name, value) -> constructorBuilder.append(objectId).append(".").append(name).append('(').append(surroundIfString(value)).append(')').append('\n'));
            if (childList != null) {
                childListItems.forEach(child -> constructorBuilder.append(objectId).append('.').append("get").append(Character.toString(childList.charAt(0)).toUpperCase()).append(childList.substring(1)).append("()").append(".add").append('(').append(((Node) child).getId()).append(')').append('\n'));
            } else if (childProperty != null) {
                constructorBuilder.append(objectId).append('.').append(childProperty).append('(').append(childObject).append(')');
            }
            if (value == root) {
                constructorBuilder.append("root = ").append(objectId).append("\n");
            }
            System.out.println(getValueAdapter().getPropertiesToSet());
        }

        private Object surroundIfString(Object object) {
        ...
        private void processValue() throws LoadException {
            ...
            if (fx_id != null) {
                ...
                //TODO: MY CHANGE
                fieldsBuilder.append("val ").append(fx_id).append(" : ").append(value.getClass().getSimpleName()).append('\n');
            }
        }
    ...
    private class InstanceDeclarationElement extends ValueElement {
        ...
        public void processAttribute(String prefix, String localName, String value)
            ...
            if (prefix != null
                    && prefix.equals(FX_NAMESPACE_PREFIX)) {
                switch (localName) {
                    case FX_VALUE_ATTRIBUTE:
                        this.value = value;
                        break;
                    case FX_CONSTANT_ATTRIBUTE:
                        constant = value;
                        break;
                    case FX_FACTORY_ATTRIBUTE:
                        factory = value;
                        break;
                    default:
                        super.processAttribute(prefix, localName, value);
    ...
    private class IncludeElement extends ValueElement {
        ...
        public ResourceBundle resources = FXMLParser.this.resources;
        public Charset charset = FXMLParser.this.charset;
                ...
                switch (localName) {
                    ...
                    case INCLUDE_RESOURCES_ATTRIBUTE:
                        resources = ResourceBundle.getBundle(value, Locale.getDefault(),
                                FXMLParser.this.resources.getClass().getClassLoader());
                        break;
        ...
        public Object constructValue() throws IOException {
            ...
            if (source.charAt(0) == '/') {
                ...
                if (FXMLParser.this.location == null) {
                    throw constructLoadException("Base location is undefined.");
                }

                location = new URL(FXMLParser.this.location, source);
            }

            FXMLParser fxmlLoader = new FXMLParser(location, resources, builderFactory, controllerFactory, charset, loaders);
            fxmlLoader.parentLoader = FXMLParser.this;

            if (isCyclic(FXMLParser.this, fxmlLoader)) {
                throw new IOException(
                        String.format(
                                "Including \"%s\" in \"%s\" created cyclic reference.",
                                fxmlLoader.location.toExternalForm(),
                                FXMLParser.this.location.toExternalForm()));
            }
    ...
    private class PropertyElement extends Element {
        ...
        public void processEndElement() throws IOException {
            ...
            System.out.println("parent for child ");

            String nodeId = (String) parent.getProperties().get("id");

            parent.childList = name;
            parent.childListItems = childListItems;
            parent.childObject = childObject;
            parent.childProperty = childProperty;
        }
    ...
    private final LinkedList<FXMLParser> loaders;
    private final StringBuilder importBuilder = new StringBuilder();
    private final StringBuilder fieldsBuilder = new StringBuilder();
    private final StringBuilder constructorBuilder = new StringBuilder();

    private ClassLoader classLoader = null;

    private FXMLParser parentLoader;
    ...
    static {
        JAVAFX_VERSION = AccessController.doPrivileged((PrivilegedAction<String>) () -> System.getProperty("javafx.version"));
    }

    public FXMLParser(URL location) {
        this(location, null, null, null, Charset.forName(DEFAULT_CHARSET_NAME), new LinkedList<>());
    }

    /*public FXMLParser(URL location, ResourceBundle resources) {
        this(location, resources, null);
    }

    public FXMLParser(URL location, ResourceBundle resources, BuilderFactory builderFactory) {
        this(location, resources, builderFactory, null);
    }

    public FXMLParser(URL location, ResourceBundle resources, BuilderFactory builderFactory, Callback<Class<?>, Object> controllerFactory) {
        this(location, resources, builderFactory, controllerFactory, );
    }
    ...
    public FXMLParser(URL location, ResourceBundle resources, BuilderFactory builderFactory, Callback<Class<?>, Object> controllerFactory, Charset charset, LinkedList<FXMLParser> loaders) {
    ...
    //TODO: Most important method in world
    public String getClassCode(String pack, String className) {
        StringBuilder sb = new StringBuilder(pack.length() + className.length() + importBuilder.length() + fieldsBuilder.length());
        //Package
        sb.append(pack).append('\n');
        //Imports
        sb.append(importBuilder);
        //Class declaration
        sb.append("public abstract class ").append(className).append(" {").append('\n');
        //Field declaration
        sb.append("val root: Parent\n");
        sb.append(fieldsBuilder);
        //Constructor declaration
        sb.append("init {").append("\n");
        sb.append(constructorBuilder);
        sb.append('}').append('\n');
        //Methods declaration
        //TODO
        //End class
    ...
    public boolean equals(Object obj) {
        if (obj instanceof FXMLParser) {
            FXMLParser loader = (FXMLParser) obj;
    ...
    public <T> T load() throws IOException {
        return loadImpl((System.getSecurityManager() != null) ? Reflection.getCallerClass() : null);
    }

    @CallerSensitive
    public <T> T load(InputStream inputStream) throws IOException {
        return loadImpl(inputStream, (System.getSecurityManager() != null) ? Reflection.getCallerClass() : null);
    }
    ...
    private void importPackage(String name) throws LoadException {
        packages.add(name);
        importBuilder.append("import ").append(name).append(".*").append('\n');
    }

    private void importClass(String name) throws LoadException {
        try {
            importBuilder.append("import ").append(name).append('\n');
            loadType(name, true);
        ...
        if (defaultClassLoader == null) {
            ...
            if (sm != null) {
                final ClassLoader callerClassLoader = (caller != null) ?
                        ...
                        null;
                if (needsClassLoaderPermissionCheck(callerClassLoader, FXMLParser.class.getClassLoader())) {
    ...
    public static void setDefaultClassLoader(ClassLoader defaultClassLoader) {
        ...
        FXMLParser.defaultClassLoader = defaultClassLoader;
    }
